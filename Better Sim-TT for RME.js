(function () {
  ...
  // GQL Responses abfangen um UUIDs zu sammeln
  ...
  async function fetchWOFromList(uuid) {
    try {
  ...
  function injectWOButtons(row, shortId) {
    ...
    if (woCache[shortId] && woCache[shortId] !== 'loading') {
      renderWOButtons(group, woCache[shortId]);
      return;
    }
    ...
    getUUID(shortId, async (uuid) => {
      if (!uuid) {
        ...
        renderWOButtons(group, null);
        return;
      }
      const wo = await fetchWOFromList(uuid);
      woCache[shortId] = wo ? { wo, eamUrl: EAM + wo } : null;
      renderWOButtons(group, woCache[shortId]);
    });
  }

  function renderWOButtons(group, result) {
    group.innerHTML = '';
  ...
  new MutationObserver(() => {
    ...
    listDebounce = setTimeout(processListRows, 200);
  }).observe(document.body, { childList: true, subtree: true });

  setTimeout(processListRows, 800);
  setTimeout(processListRows, 2200);
